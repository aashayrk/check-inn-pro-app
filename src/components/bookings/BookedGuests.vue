  ...
  <div>
    <info-section section-title="Guests">
      <template #actions>
        ...
        <!-- add guest -->
        <!-- <modal-dialog v-if="booking" :disabled="bookedGuests.length === props.booking.package.adult_count_max || props.booking.checked_in === 1 || props.booking.no_show === 1 || props.booking.cancelled === 1 || userAbilities.deny('create', 'guests')">
          <template #trigger>
            <button class="btn btn-primary ml-1" title="Add Guest" :disabled="bookedGuests.length === props.booking.package.adult_count_max || props.booking.checked_in === 1 || props.booking.no_show === 1 || props.booking.cancelled === 1 || userAbilities.deny('create', 'guests')">
              <span>Add Guest</span>
          ...
          <template #default="defaultProps">
            <info-section section-title="Add Guest">
              <add-guest :check-in="booking" @close="onGuestAdded(defaultProps, $event)"></add-guest>
            </info-section>
          </template>
        </modal-dialog> -->
      </template>
      <template #default>
        <div v-if="bookedGuestsReq.success">
          <div v-for="(item, index) in bookedGuests" :key="index" class="row-item">
            <div class="data-row">
              <div class="data-info">
                <div class="row">
        ...
        <div v-else>
          <loading-dots :progress="bookedGuestsReq.progress" :message="bookedGuestsReq.message"></loading-dots>
        </div>
      </template>
    </info-section>
  </div>
...
// import PreviewDocument from '@/vue/PreviewDocument.vue';
...
let bookedGuests = ref([]);
let bookedGuestsReq = reactive(useApi());
let removeReq = reactive(useApi());
let deletePrefReq = reactive(useApi());

function getGuests() {
  bookedGuestsReq.send(
    `/api/client/v1/bookings/${props.bookingId}/guests`
  )
  .then (res => {
    if (res) {
      bookedGuests.value = res.guests;
    }
  });
}
...
function onGuestRemovalConfirmed(guest, dialogProps) {
  removeReq.send(
    `/api/client/v1/bookings/${props.bookingId}/guests/${guest.id}`,
    'DELETE', 
  )
  .then (res => {
    if (res) {
