<template>
  <div>
    ...
    <info-section section-title="Special Instructions">
      <!-- <template #actions>
        <modal-dialog :disabled="props.booking && (props.booking.checked_in === 1 || props.booking.no_show === 1 || props.booking.cancelled === 1) || userAbilities.deny('createInstruction', 'bookings')">
          <template #trigger>
            <button class="btn btn-primary" :disabled="props.booking && (props.booking.checked_in === 1 || props.booking.no_show === 1 || props.booking.cancelled === 1) || userAbilities.deny('createInstruction', 'bookings')">
          ...
          <template #default="defaultProps">
            <info-section section-title="Add Instruction">
              <add-instruction :booking="booking" @close="onInstructionAdded(defaultProps, $event)"></add-instruction>
            </info-section>
          </template>
        </modal-dialog>
      </template> -->
...
function getInstructions() {
  getInstructionsReq.send(
    `/api/client/v1/bookings/${props.bookingId}/instructions`
  )
  .then (res => {
    if (res) {
...
function deleteInstruction(handle, instruction) {
  getInstructionsReq.send(
    `/api/client/v1/bookings/${props.bookingId}/instructions/${instruction.id}`,
    'DELETE'
  )
  .then (res => {
    if (res) {
