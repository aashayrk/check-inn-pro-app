<template>
  <div class="rounded-xl h-full relative overflow-hidden" :class="props.day.isToday && ! props.room.clean ? 'bg-yellow-300' : 'bg-green-200'">
    <div class="absolute top-0 flex h-full bg-red-200" :class="{'border-l border-white': props.room.allocations[0].starts, 'border-r border-white': props.room.allocations[0].ends}" :style="placeAllocation(props.room.allocations[0])"></div>
    ...
    <div class="p-6 relative flex flex-col h-full">
      <div class="flex justify-between flex-grow">
        ...
        <!-- info -->
        <modal-dialog dialog-title="CheckIn Summary">
          <template #trigger>
          ...
          <template #default="defaultProps">
            <check-in-summary :check-in-id="room.allocations[0].check_in_id"></check-in-summary>
      ...
      <div class="p-4 -m-4 mt-6 rounded-xl bg-white bg-opacity-75">
        <p class="leading-none">{{ props.room.allocations[0].check_in.guests[0].guest.name }}</p>
