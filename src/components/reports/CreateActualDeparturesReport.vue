<template>
  <modal-dialog size="sm" class="ml-1" dialog-title="Actual Departures Report Options">
    <template #trigger>
...
let actions = reactive(useReports('Actual Departures Report'));

function createReport(callback) {
  createReportReq.send(
    `/api/client/v1/reports/actual-departures`,
    'POST',
    {
      report: props.report,
      from: filters.from,
